...
from cat_images.subnet import MinerPayload, MinerResult, ValidationResult

log = logging.getLogger("validation-algorithm")


def validate(
    batch_to_validate: BatchedTaskInputOutput[MinerPayload, MinerResult],
    ...
    for task_input_output in batch_to_validate.task_input_outputs:
        log.info(
            f"Validating task result {task_input_output.task_result_id} with input {task_input_output.task_input} and output {task_input_output.task_output}"
        )
